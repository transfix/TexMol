...
void Skel::buildHelixGeometry(Geometry* helixGeom)
{
	...
	FILE* fp =fopen("helix.rawc", "w");
	fprintf(fp, "%d %d\n", helixGeom->m_NumTriVerts, helixGeom->m_NumTris);
	for(int i=0; i< helixGeom->m_NumTriVerts; i++)
	{
		fprintf(fp, "%f %f %f %f %f %f\n", helixGeom->m_TriVerts[3*i +0], helixGeom->m_TriVerts[3*i +1], helixGeom->m_TriVerts[3*i +2], helixGeom->m_TriVertColorsTransparent[4*i+0],
		 helixGeom->m_TriVertColorsTransparent[4*i+1], helixGeom->m_TriVertColorsTransparent[4*i+2]); 
	}
...
/* //  Not the skeleton written
	FILE* fp =fopen("curve.rawc", "w");
