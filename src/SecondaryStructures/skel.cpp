...
// arand, 8-24-2011
// also moved these here because the mac wasn't linking correctly when these were defined in the class header...
...
void Skel::add_u2_to_skel(const pair< vector< vector<Cell_handle> >, vector<Facet> >& u2)
{
	...
	cout<<"edge_list.size()  " << (int)edge_list.size() << endl;
	cout<<"vert_list.size(): " << (int)vert_list.size() << endl;
	set_ne((int)edge_list.size());
	set_nv((int)vert_list.size());
}
...
void Skel::buildSheetGeometry(Geometry* geom)
{
	...
	{
		fprintf(fp, "%f %f %f %f %f %f\n", geom->m_TriVerts[3*i +0], geom->m_TriVerts[3*i +1], geom->m_TriVerts[3*i +2], geom->m_TriVertColorsTransparent[4*i+0],
		 geom->m_TriVertColorsTransparent[4*i+1], geom->m_TriVertColorsTransparent[4*i+2]); 
	}
	...
	write_sheet_wrl(geom,"sheet.wrl");
	}
}
...
{
	cout<<"alphaMinWidth and alphaMaxWidth are: " << alphaMinWidth <<" " << alphaMaxWidth << endl;
