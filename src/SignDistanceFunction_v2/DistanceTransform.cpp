...
DistanceTransform::DistanceTransform(FaceVertSet3D& fvs, int dim[3], float dist, float sx, float sy, float sz)
{
	float factor[3];
	factor[0] = sx;
	factor[1] = sy;
...
DistanceTransform::DistanceTransform(FaceVertSet3D& fvs, int dim[3], float bbox[3], float spacing[3]) {
	...
	for(int kk = -1; kk <= 0; kk++)
	{
		for(int jj = -1; jj <= 0; jj++)
		{
			for(int ii = -1; ii <= 0; ii++)
			{
				...
				for(int i = 0; i < p_DistanceTransformCells[nc].triList.length(); i++)
				{
...
int DistanceTransform::inOrOut(const dynamic_array<int> &triList, const Point3f& vert, const Point3f& nearPnt)
{
	int i, j, len = triList.length();
	assert(len > 1);
	Point3f center, cpnt;
...
bool DistanceTransform::pointInTriangle(const Point3f& res, const Point3f& vert0, const Point3f& vert1, const Point3f& vert2, const Vector3f& norm)
{
	double alpha, beta;
...
bool DistanceTransform::intersectDistanceTransformCell(const Point3f& v0, const Vector3f& norm, int i, int j, int k)
{
