...
void resContFilter::getScores(vector<Point> &sAtoms, vector<Point> &mAtoms, vector<double> &sAtomValues, vector<double> &mAtomValues, double &val) 
{
	Matrix trans;
	trans.reset();
	getScores(trans, sAtoms, mAtoms, sAtomValues, mAtomValues, val);
}

void resContFilter::getScores( Matrix transMat, vector<Point> &sAtoms, vector<Point> &mAtoms, vector<double> &sAtomValues, vector<double> &mAtomValues, double &val)
{
   ...
   if ( farEnough ) return;
      ...
      {
		   double *staticAtomsValue = new double[ nStaticAtoms ];
		   double *movingAtomsValue = new double[ nMovingAtoms ];
		   for ( int i = 0; i < nStaticAtoms; i ++)
			   staticAtomsValue[ i ] = 0;
		   for ( int i = 0; i < nMovingAtoms; i ++)
			   movingAtomsValue[ i ] = 0;
           ...
           for ( int i = 0; i < nMovingAtoms; i++ )
              ...
              for ( int j = 0; j < nStaticAtoms; j++ )
                {
                 ...
                 if ( d2 <= distCutoffSQ && (intVal[ rIDM ][ rIDS ] > 0 || intVal[ rIDM ][ rIDS ] < -3.0))
                   {
					 staticAtomsValue[ j ] += intVal[ rIDM ][ rIDS ];
					 movingAtomsValue[ i ] += intVal[ rIDM ][ rIDS ];
                   }
                }
		     ...
		     for (int i = 0; i < nMovingAtoms; i ++)
			 {
				 mAtoms.push_back(Point(movingAtoms[ i ].x, movingAtoms[ i ].y, movingAtoms[ i ].z));
				 mAtomValues.push_back(movingAtomsValue[ i ]);
				val += movingAtomsValue[ i ];
			 }
		     for (int i = 0; i < nStaticAtoms; i ++)
			 {
				 sAtoms.push_back(Point(staticAtoms[ i ].x, staticAtoms[ i ].y, staticAtoms[ i ].z));
				 sAtomValues.push_back(staticAtomsValue[ i ]);
				val += staticAtomsValue[ i ];
			 }
			 freeMem( staticAtomsValue );
			 freeMem( movingAtomsValue );
      }
}
