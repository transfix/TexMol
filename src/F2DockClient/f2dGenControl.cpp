...
void F2dGenControl::init()
{

}

bool F2dGenControl::setReceptorPDBName(string fileName)
{
	return basPar->setReceptorPDBName(fileName);
}

bool F2dGenControl::setLigandPDBName(string fileName)
{
	return basPar->setLigandPDBName(fileName);
}

bool F2dGenControl::setResolution(int index)
{
	return f2dPar->setResolution(index);
}

bool F2dGenControl::setJobTitle(string name)
{
	return basPar->setJobTitle(name);
}

bool F2dGenControl::handleJob(bool p)
{
	if(basPar->getReceptorPDBAvailable() && basPar->getLigandPDBAvailable() && f2dPar->prepareInputFile(basPar->getF2dGenInputFileName()))
	{
		basPar->setF2dGenInputAvailable(true);

		if(p)	basPar->setStoreReceptorPQR(true);
		if(p)	basPar->setStoreLigandPQR(true);
		basPar->setStoreReceptorF2d(true);
		basPar->setStoreLigandF2d(true);
...
bool F2dGenControl::setParamsFromFile(string fileName)
{
	...
	return basPar->setF2dGenInputName(fileName2,0);
}
