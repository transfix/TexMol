...
/* $Id: HDF5_IO.cpp 6567 2013-02-18 15:47:24Z edwardsj $ */
...
namespace
{
  ...
  public:
    ...
    void operator()()
    {
      ...
      LOG4CPLUS_INFO(logger, "Building hierarchy...");
          ...
          while(1)
            {
              ...
              if(isDirty)
                {
                  LOG4CPLUS_DEBUG(logger, "computing " << hier_volume_name);
...
namespace VolMagick
{
  ...
  {
    static log4cplus::Logger logger = log4cplus::Logger::getInstance("HDF5_IO.readVolumeFile");
    ...
    if(isGroup(actualFileName,objectName))
      {
        ...
        BOOST_FOREACH(std::string obj, hierarchy_objects)
          {
            LOG4CPLUS_TRACE(logger, "Reading " << obj);
        ...
        LOG4CPLUS_TRACE(logger, "Selected object " << hierarchy_object);
      ...
      {
        if(var > 0 || time > 0)
          LOG4CPLUS_WARN(logger, "WARNING - HDF5_IO doesn't yet support lone datasets "
                         "with more than one variable or timestep!");
        var = 0; time = 0;
        volume_name = objectName;
      }
    ...
    boost::tie(data,dim) = 
      readDataSet(actualFileName, volume_name,
                  subvolbox, vol.voxelType(), maxdim);
