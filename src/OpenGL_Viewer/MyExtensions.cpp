...
bool MyExtensions::extensionExists(const char *extension)
{
    ...
    {
        if(ext == *iter) {
            fprintf( stderr, "GL_EXT_framebuffer_object is supported\n"); break; }
        else
            ++iter;
    }
}
...
bool MyExtensions::initExtensions(const char* requestedExtensions)
{
	...
	if (!extensions)
	{
		fprintf( stderr, "getExtension failed\n");
	...
	for (currentExt = reqExts; (currentExt = (char*)EatWhiteSpace(currentExt)) && *currentExt; currentExt = (char*)EatNonWhiteSpace(currentExt))
	{
		...
		*extEnd = (char)0;
	//	bool exists = extensionExists(currentExt, paddedExtensions);
		bool exists = extensionExists(currentExt);
		bool inits = initExtension(currentExt);
		if ( !exists )
		{
			fprintf( stderr, "%s extension does not exist\n", currentExt);
			success = false;
		}
		if ( !inits )
		{
			fprintf( stderr, "%s extension init failed\n", currentExt);
			success = false;
		}
		*extEnd = saveChar;
	}
...
// get pointers to GL functions
        	INIT_WGL_PROC_POINTER(MYPFNGLGENFRAMEBUFFERSEXTPROC, glGenFramebuffersEXT);
        	INIT_WGL_PROC_POINTER(MYPFNGLDELETEFRAMEBUFFERSEXTPROC, glDeleteFramebuffersEXT);
        	INIT_WGL_PROC_POINTER(MYPFNGLBINDFRAMEBUFFEREXTPROC, glBindFramebufferEXT);
        	INIT_WGL_PROC_POINTER(MYPFNGLCHECKFRAMEBUFFERSTATUSEXTPROC, glCheckFramebufferStatusEXT);
        	INIT_WGL_PROC_POINTER(MYPFNGLGETFRAMEBUFFERATTACHMENTPARAMETERIVEXTPROC, glGetFramebufferAttachmentParameterivEXT);
        	INIT_WGL_PROC_POINTER(MYPFNGLGENERATEMIPMAPEXTPROC, glGenerateMipmapEXT);
        	INIT_WGL_PROC_POINTER(MYPFNGLFRAMEBUFFERTEXTURE2DEXTPROC, glFramebufferTexture2DEXT);
       		INIT_WGL_PROC_POINTER(MYPFNGLFRAMEBUFFERRENDERBUFFEREXTPROC, glFramebufferRenderbufferEXT);
        	INIT_WGL_PROC_POINTER(MYPFNGLGENRENDERBUFFERSEXTPROC, glGenRenderbuffersEXT);
        	INIT_WGL_PROC_POINTER(MYPFNGLDELETERENDERBUFFERSEXTPROC, glDeleteRenderbuffersEXT);
        	INIT_WGL_PROC_POINTER(MYPFNGLBINDRENDERBUFFEREXTPROC, glBindRenderbufferEXT);
        	INIT_WGL_PROC_POINTER(MYPFNGLRENDERBUFFERSTORAGEEXTPROC, glRenderbufferStorageEXT);
        	INIT_WGL_PROC_POINTER(MYPFNGLGETRENDERBUFFERPARAMETERIVEXTPROC, glGetRenderbufferParameterivEXT);
        	INIT_WGL_PROC_POINTER(MYPFNGLISRENDERBUFFEREXTPROC, glIsRenderbufferEXT);
		return true;
