...
GroupOfGBAtoms::GroupOfGBAtoms()
{
}

GroupOfGBAtoms::GroupOfGBAtoms(GroupOfGBAtoms* m)
{
    int i;
...
void GroupOfGBAtoms::addAtom(GBAtom* currentAtom)
{
    GBAtom* a = new GBAtom(currentAtom);
    m_Atoms.push_back(a);
}

void GroupOfGBAtoms::getBornRadii(Surface *surface)
{
	int i,k;
...
cout << "Born radii using trivial way with correction" << endl;
/*
...
*/
        ...
        for (i = 0; i < numOfAtoms; i++)
        {
                sum[i] = 0;     sumx[i] = 0;    sumy[i] = 0;    sumz[i] = 0;
...
//                A7 = sqrt(A7); //A7 = sqrt(A7); // wenqi added on 12/17
		A7 = sqrt(A7); A7 = sqrt(A7); // wenqi added on 10/18/08
                if( A4 + A7 > 0.0000001)
...
cout << "Born radii using fast summation w/o correction" << endl;
...
*/
	...
	for (i = 0; i < numOfAtoms; i++)
	{
		sum[i] = 0.0;	sumx[i] = 0.0;	sumy[i] = 0.0;	sumz[i] = 0.0;
	}
	param /= (1*abs_max);
...
// with correction
#if defined(_FASTSUM_) && defined(_CORRECTION_)
cout << "Born radii using fast summation with correction" << endl;
...
*/
	...
	{
		sum[i] = 0.0;	sumx[i] = 0.0;	sumy[i] = 0.0;	sumz[i] = 0.0;
		c_sum[i] = 0.0;	c_sumx[i] = 0.0;	c_sumy[i] = 0.0;	c_sumz[i] = 0.0;
	}
	param /= (1*abs_max);
	m_fastsum = new FastSummation(3, N, M, n, m, p, (char *)"one_over_quartic", (char *)"one_over_seventh", &param, eps_I, eps_B);
	// arand: line below causes crash...
	m_fastsum->threadedFastSumCorrection( x, y, coeff, coeffx, coeffy, coeffz, sum, sumx, sumy, sumz, c_sum, c_sumx, c_sumy, c_sumz);
        // test for longhorn - cha
//	m_fastsum->threadedFastSumCorrection(8, x, y, coeff, coeffx, coeffy, coeffz, sum, sumx, sumy, sumz, c_sum, c_sumx, c_sumy, c_sumz);
	delete m_fastsum;
//	for (i = 0; i < numOfAtoms; i++)
...
//		printf("%f %f\n", A4, sqrt(sqrt(A7)));
                if(A7 < 0.0) A7 = 0.0;
//                A7 = sqrt(A7); //A7 = sqrt(A7); // wenqi added on 12/17
                A7 = sqrt(A7); A7 = sqrt(A7); // wenqi modified on 10/18/08
                if( A4 + A7 > 0.0000001)
                        m_Atoms[i]->Born_radius = 1.0 / ( (1.0-1.0/sqrt(2.0)) * A4 + A7 ); // this is GBSW model
                else
                        m_Atoms[i]->Born_radius = 100000.0;
...
double GroupOfGBAtoms::dotProduct(double *u, double *v)
{
	return u[0]*v[0] + u[1]*v[1] + u[2]*v[2];
}


void GroupOfGBAtoms::getNeighbors()
{
	int i, j;
...
bool GroupOfGBAtoms::overlap(int i, int j)
{
	double xi[3], xj[3];
	double ri, rj;
	double dij;
...
void GroupOfGBAtoms::assignBRtoChild(GroupOfGBAtoms *mol)
{
        int i, j;
        for (i = 0; i < numOfAtoms; i++)
        {
