...
void Geometry::CalculateQuadFlatNormals()
{
	...
	for(c=0; c<m_NumQuads; c++)    // for every quad
	{
		...
		add(normal1, normal2);
		normalize(normal1);
		setVector(m_QuadFlatVerts+c*12+0*3, m_QuadVerts+v0*3); // set the verts
		...
		{
			setVector(m_QuadFlatTexCoords+c*12+0*3, m_QuadVertTexCoords+v0*3); // set the tex coords
			setVector(m_QuadFlatTexCoords+c*12+1*3, m_QuadVertTexCoords+v1*3);
			setVector(m_QuadFlatTexCoords+c*12+2*3, m_QuadVertTexCoords+v2*3);
			setVector(m_QuadFlatTexCoords+c*12+3*3, m_QuadVertTexCoords+v3*3);
		}
...
void Geometry::CalculateTriFlatNormals()
{
	...
	for(c=0; c<m_NumTris; c++)    // for every triangle
	{
		...
		CalculateTriangleNormal(normal, v0, v1, v2);
		normalize(normal);
		setVector(m_TriFlatVerts+c*9+0*3, m_TriVerts+v0*3); // set the verts
		setVector(m_TriFlatVerts+c*9+1*3, m_TriVerts+v1*3);
		setVector(m_TriFlatVerts+c*9+2*3, m_TriVerts+v2*3);
		if(m_TriVertTexCoords)
		{
			setVector(m_TriFlatTexCoords+c*9+0*3, m_TriVertTexCoords+v0*3); // set the tex coords
			setVector(m_TriFlatTexCoords+c*9+1*3, m_TriVertTexCoords+v1*3);
			setVector(m_TriFlatTexCoords+c*9+2*3, m_TriVertTexCoords+v2*3);
		}
...
bool Geometry::separateComponents(vector<Geometry*>* components)
{
	...
	while(true)
	{
		...
		if(! getNextComponentSize(&numVerts, &numTris, &nextComponentIndex, componentIndex))
		{
			break;
		}
...
Geometry* Geometry::merge(Geometry* geometry)
{
	...
	if(m_TriVertColorsTransparent && geometry->m_TriVertColorsTransparent)
	{
		for(i=0; i<m_NumTriVerts; i++)
		{
			mergedGeometry->m_TriVertColorsTransparent[4*i+0] = m_TriVertColorsTransparent[4*i+0];
			mergedGeometry->m_TriVertColorsTransparent[4*i+1] = m_TriVertColorsTransparent[4*i+1];
			mergedGeometry->m_TriVertColorsTransparent[4*i+2] = m_TriVertColorsTransparent[4*i+2];
			mergedGeometry->m_TriVertColorsTransparent[4*i+3] = m_TriVertColorsTransparent[4*i+3];
		}
		for(i=m_NumTriVerts; i<m_NumTriVerts+geometry->m_NumTriVerts; i++)
		{
			mergedGeometry->m_TriVertColorsTransparent[4*i+0] = geometry->m_TriVertColorsTransparent[4*(i-m_NumTriVerts)+0];
			mergedGeometry->m_TriVertColorsTransparent[4*i+1] = geometry->m_TriVertColorsTransparent[4*(i-m_NumTriVerts)+1];
			mergedGeometry->m_TriVertColorsTransparent[4*i+2] = geometry->m_TriVertColorsTransparent[4*(i-m_NumTriVerts)+2];
			mergedGeometry->m_TriVertColorsTransparent[4*i+3] = geometry->m_TriVertColorsTransparent[4*(i-m_NumTriVerts)+3];
		}
	}
...
bool Geometry::use2DTriangleTexture()
{
	return 	m_Use2DTriangleTexture;
}
...
bool Geometry::printTriangleDerivatives(FILE* fp)
{
	...
	if(!m_TriVertNormals || !m_TriMeanCurv || !m_TriGaussianCurv || !m_K1 || !m_K2)
	{
		return false;
	}
	int i;
...
void Geometry::setTriangleColorsByNormals()
{
	...
	for(i=0; i<m_NumTriVerts; i++)
	{
		m_TriVertColorsTransparent[i*4+0] = m_TriVertNormals[i*3+0]/2.0 + 0.5;
		m_TriVertColorsTransparent[i*4+1] = m_TriVertNormals[i*3+1]/2.0 + 0.5;
		m_TriVertColorsTransparent[i*4+2] = m_TriVertNormals[i*3+2]/2.0 + 0.5;
	}
}
...
bool Geometry::set2DTexture(int width, int height, unsigned char* bits, int numBytesPerEntry)
{
...
double Geometry::getHausdorffDistance(float p11[3], float p12[3], float p13[3], float p21[3], float p22[3], float p23[3])
{
...
void Geometry::allocateBufferObjects()
{
	...
	{
		fprintf(stderr, "ERROR: Geometry needs VBOs/OpenGL 1.5 to work properly.\n");
		m_UsingBuffers = false;
	}
	else
	{
...
void Geometry::prepareBuffersTris()
{
	if(!m_BuffersInitialized)
	{
		...
		if(m_UsingBuffers)
		{
			...
			m_Extensions->glBufferDataARB(GL_ARRAY_BUFFER_ARB, m_NumTriVerts*3*sizeof(float), m_TriVerts, GL_STATIC_DRAW_ARB);
			m_Extensions->glBindBufferARB(GL_ARRAY_BUFFER_ARB, m_NormalObject);
			m_Extensions->glBufferDataARB(GL_ARRAY_BUFFER_ARB, m_NumTriVerts*3*sizeof(float), m_TriVertNormals, GL_STATIC_DRAW_ARB);
			m_Extensions->glBindBufferARB(GL_ARRAY_BUFFER_ARB, m_ColorObject);
			m_Extensions->glBufferDataARB(GL_ARRAY_BUFFER_ARB, m_NumTriVerts*4*sizeof(float), m_TriVertColorsTransparent, GL_STATIC_DRAW_ARB);
...
void Geometry::bindColorBuffer(bool colorsChanged)
{
	...
	if(m_UsingBuffers)
	{
		...
		if(colorsChanged)
		{
			m_Extensions->glBufferDataARB(GL_ARRAY_BUFFER_ARB, m_NumTriVerts*4*sizeof(float), m_TriVertColorsTransparent,
										  GL_DYNAMIC_DRAW_ARB);
		}
...
std::vector<CCVOpenGLMath::Vector> triangleCentersNDC; // normalized depth coordinates
...
CCVOpenGLMath::Vector Geometry::triangleIndexToCenter(int triangleIndex)
{
	...
	CCVOpenGLMath::Vector triangleVertex1(m_TriVerts[triangleVertex1Index*3 + 0],
										  m_TriVerts[triangleVertex1Index*3 + 1],
										  m_TriVerts[triangleVertex1Index*3 + 2],
										  1);
	CCVOpenGLMath::Vector triangleVertex2(m_TriVerts[triangleVertex2Index*3 + 0],
										  m_TriVerts[triangleVertex2Index*3 + 1],
										  m_TriVerts[triangleVertex2Index*3 + 2],
										  1);
	CCVOpenGLMath::Vector triangleVertex3(m_TriVerts[triangleVertex3Index*3 + 0],
										  m_TriVerts[triangleVertex3Index*3 + 1],
										  m_TriVerts[triangleVertex3Index*3 + 2],
										  1);
	CCVOpenGLMath::Vector returnValue = (triangleVertex1+triangleVertex2+triangleVertex3) * (1.0f/3);
...
void Geometry::sortGeometryTris()
{
	...
	CCVOpenGLMath::Matrix projectionMatrix(
	...
	if(!m_SortInitialized)
	{
		...
		std::sort(m_DepthOrderedTriangles.begin(), m_DepthOrderedTriangles.end(), triangleDepthCompareFunctor);
	...
	for(int i=0; i<m_DepthOrderedTriangles.size(); i++)
	{
		m_DepthOrderedIndices[3*i + 0] = m_Tris[m_DepthOrderedTriangles[i]*3 + 0];
		m_DepthOrderedIndices[3*i + 1] = m_Tris[m_DepthOrderedTriangles[i]*3 + 1];
		m_DepthOrderedIndices[3*i + 2] = m_Tris[m_DepthOrderedTriangles[i]*3 + 2];
	}
}

//Craig: Really doesn't belong here, but currently the situation is unreformable
// should look like:
// Point a; Point b;
// (a-b).norm();
...
double Geometry::getArea(float* point, double radius)
{
	...
	for(int i=0; i<m_NumTris; i++)
	{
		...
		int v2 = m_Tris[3*i+1];
		int v3 = m_Tris[3*i+2];
		area += CCVOpenGLMath::AreaVolumes::getTriangleArea(m_TriVerts[3*v1+0],
				m_TriVerts[3*v1+1],
				m_TriVerts[3*v1+2],
				m_TriVerts[3*v2+0],
				m_TriVerts[3*v2+1],
