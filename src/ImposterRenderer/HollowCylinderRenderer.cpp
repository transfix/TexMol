...
#ifdef CG
#include <ImposterRenderer/cgGLHollowCylinderFragmentShader.h>
#include <ImposterRenderer/cgGLHollowCylinderVertexShader.h>
#include <ImposterRenderer/GlobalCGContext.h>
#endif
#ifdef GLSL
#include <ImposterRenderer/HollowCylinderFragmentShader.glsl>
#include <ImposterRenderer/HollowCylinderVertexShader.glsl>
...
void HollowCylinderRenderer::initDefaults()
{
	m_Initialized = false;
...
void HollowCylinderRenderer::initRenderer(bool drawFunction)
{
	...
	{
		m_Initialized = false;
		fprintf(stderr, "Hollow: ERROR: OpenGL extensions unavailable, update your graphics drivers.\n");
		return;
	}
	// if FP30 isn't present we will use a different branch of every render method
	if (!initCG(drawFunction))
	{
		fprintf(stderr, "Hollow: WARNING: FP30/FP40 not found, falling back on GL Rendering.\n");
		m_UseGLFallback = true;
	}
	if (!loadProgram())
	{
		fprintf(stderr, "Hollow: load program error, falling back on GL Rendering.\n");
		m_UseGLFallback = true;
	}
	m_Initialized = true;
}

void HollowCylinderRenderer::bindProgramAndParamsCg()
{
	#ifdef CG
...
void HollowCylinderRenderer::bindProgramAndParamsGLSL()
{
	#ifdef GLSL
	_shader->bind();
...
void HollowCylinderRenderer::bindProgramAndParams()
{
	#ifdef CG
...
void HollowCylinderRenderer::unbindProgramCg()
{
	#ifdef CG
...
void HollowCylinderRenderer::unbindProgramGLSL()
{
	#ifdef GLSL
...
void HollowCylinderRenderer::unbindProgram()
{
	#ifdef CG
	unbindProgramCg();
	#endif
...
void HollowCylinderRenderer::bindMatrices()
{
	#ifdef CG
...
bool HollowCylinderRenderer::loadProgramCg()
{
	#ifdef CG
	...
	std::cout << "HollowCylRender: VP load" << std::endl;
	...
	std::cout << "HollowCylRender: FP load" << std::endl;
	#endif
	return true;
}

bool HollowCylinderRenderer::loadProgramGLSL()
{
	#ifdef GLSL
	// create shader object
        _shader        = new glslShader();
	_shader->initFromString( glslHollowCylVertexShader, glslHollowCylFragmentShader );
...
bool HollowCylinderRenderer::loadProgram()
{
	#ifdef CG
...
bool HollowCylinderRenderer::initCG(bool drawFunction)
{
                ...
                if (cgGLIsProfileSupported(CG_PROFILE_ARBFP1))
                        ...
                        return false;
                std::cout << "HollowCylinderRender: Using ARB profile for Cg" << std::endl;
                #else
		...
		if (rotations && translations && (numberOfTransformations>0))
		{
			for (int i=0; i<numberOfTransformations; i++)
			{
				...
				*/
				...
				glDrawArrays(GL_QUADS, 0, m_NumberOfCylinders*4);
				glPopMatrix();
			}
		}
		else
