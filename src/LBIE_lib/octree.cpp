...
void Octree::pdbVolume(const char* pdb_fname)
{
	...
	goaBlur->generateBlurmap(mol, volumeData, PDBParser::GroupOfAtoms::ELECTRON_DENSITY, 0, false, PDBParser::GroupOfAtoms::ATOM, PDBParser::GroupOfAtoms::VDW_RADIUS, PDBParser::ATOM_TYPE);
...
void Octree::put_qef(int oc_id, double* sigma_ni_2, double* sigma_ni_2_pi,double* sigma_ni_2_pi_2,double* solution,double qef)
{
	...
	{
		qef_array[oc_id]=(double*)mallocSafely(sizeof(double)*(3+3+3+3+1));
	}
...
void Octree::put_qef_in(int oc_id, double* sigma_ni_2, double* sigma_ni_2_pi,double* sigma_ni_2_pi_2,double* solution,double qef)
{
  ...
  cout << "arand " << qef_array_in[oc_id] << " " << oc_id << endl;
	...
	{
		qef_array_in[oc_id]=(double*)mallocSafely(sizeof(double)*(3+3+3+3+1));
	}
...
int Octree::is_min_edge(int oc_id, int e_id, unsigned int* vtx, int& vtx_num, int intersect_id, Geoframe& geofrm)
{
	...
	{
		case 0 :
			if(is_refined(x,y,z-1,level) || is_refined(x,y-1,z-1,level) || is_refined(x,y-1,z,level))
			{
				return 0;
			}
			...
			break;
		case 1 :
			if(is_refined(x,y-1,z,level) || is_refined(x+1,y-1,z,level) || is_refined(x+1,y,z,level))
			{
				return 0;
			}
		...
		case 4 :
			...
			break;
		case 5 :
			if(is_refined(x,y+1,z,level) || is_refined(x+1,y,z,level) || is_refined(x+1,y+1,z,level))
			{
				return 0;
			}
			temp_vtx[1]=min_vtx(x,y+1,z,level,geofrm);
		...
		case 6 :
			...
			if(is_refined(x,y+1,z,level) || is_refined(x,y+1,z+1,level) || is_refined(x,y,z+1,level))
			{
				return 0;
			}
			temp_vtx[1]=min_vtx(x,y+1,z,level,geofrm);
			...
			break;
		case 7 :
			if(is_refined(x-1,y,z,level) || is_refined(x-1,y+1,z,level) || is_refined(x,y+1,z,level))
			{
				return 0;
			}
			temp_vtx[1]=min_vtx(x-1,y,z,level,geofrm);
			...
			break;
		case 8 :
			if(is_refined(x,y,z-1,level) || is_refined(x-1,y,z-1,level) || is_refined(x-1,y,z,level))
			{
				return 0;
			}
			temp_vtx[1]=min_vtx(x-1,y,z,level,geofrm);
...
void Octree::mesh_extract(Geoframe& geofrm, float err_tol)
{
	...
	flag_extend = 1;
	int func = 1;
	BSplineCoeff = (float*)mallocSafely(sizeof(float)*dim[0]*dim[1]*dim[2]);
...
void Octree::quality_improve(Geoframe& geofrm, int improve_method,float delta_t)
{
...
int Octree::is_intersect(int in_idx ,float isovalue , float* in_value, int& iv_idx , int i, int j, int k , int level , int faceidx, Geoframe& geofrm)
{
	float pt[3], norm[3];
...
void Octree::interpRect3Dpts_y(int i1, int j1, int k1, float d1, float d2, float val, float* pt,float* norm,int level)
{
	double ival,gval;
	int cell_size;
	float g1[3],g2[3];
...
void Octree::interpRect3Dpts_z(int i1, int j1, int k1, float d1, float d2, float val, float* pt,float* norm,int level)
{
	double ival,gval;
	int cell_size;
	float g1[3],g2[3];
...
void Octree::edge_contraction(Geoframe& geofrm)
{
	...
	for(index = 0; index < maxIndex; index++)
	{
	  //fprintf(output, "Loop %d\n", index);
		...
		if(fscanf(input, "%d %d\n", &nv, &ntet) != 2) error("Error reading number of vertices and quads!");
		...
		for(i = 0; i < nv; i++)
		{
			if(fscanf(input,"%f %f %f %f %f %f %d\n", &vx, &vy, &vz, &nx, &ny, &nz, &b_sign) != 7) error("Error reading vertex!");
			vtx[i][0] = vx;
			vtx[i][1] = vy;
			vtx[i][2] = vz;
			normal[i][0] = nx;
		...
		for(i = 0; i < ntet; i++)
		{
			...
			if(fscanf(input, "%d %d %d %d\n", &v0, &v1, &v2, &v3) != 4) error("Could not read vertex!");
...
//			sum0+sum1+sum2+sum3+sum4+sum5+sum6+sum7+sum8+sum9+sum10+sum11+sum12+sum13+sum14+sum15);
	input = fopen("input.raw", "r");
	if(fscanf(input, "%d %d\n", &nv, &ntet) != 2) error("Could not read the number of vertices and quads!");
	geofrm.setNumVerts(nv);
	geofrm.setNumTris(ntet*4);
	for(i = 0; i < nv; i++)
	{
		if(fscanf(input,"%f %f %f %f %f %f %d\n", &vx, &vy, &vz, &nx, &ny, &nz, &b_sign) != 7) error("Error reading vetex!");
		geofrm.verts[i][0] = vx;
		geofrm.verts[i][1] = vy;
		geofrm.verts[i][2] = vz;
	...
	for(i = 0; i < ntet; i++)
	{
		if(fscanf(input,"%d %d %d %d\n", &v0, &v1, &v2, &v3) != 4) error("Error reading quad!");
		for(j = 0; j < 3; j++)
		{
...
//			cond_min, temp0/num, cond_max, oddy_min, temp1/num, oddy_max, a_vert);
	...
	for(index = 0; index < maxIndex; index++)
	{
		...
		if(fabs(cond_max1 - cond_max) < 1.0e-4 && cond_min > 0.0f && cond_max < 10.0f)
		{
			break;
		}
		cond_max1 = cond_max;
