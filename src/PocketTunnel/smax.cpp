...
void grow_maximum(Cell_handle c_max, Triangulation& triang, map<int, cell_cluster> &cluster_set)
{
	// mark it visited.
	c_max->visited = true;
...
void club_contiguous_segment(Triangulation& triang, map<int, cell_cluster> &cluster_set)
{
...
void calc_cluster_volume_and_store_with_cluster_rep(Triangulation& triang, map<int, cell_cluster> &cluster_set,	vector<double> &cluster_volume_vector, vector<int> &cluster_rep_vector)
{
	for(FCI cit = triang.finite_cells_begin(); cit != triang.finite_cells_end(); cit ++)
	{
...
void sort_cluster_wrt_volume(const vector<double>& cluster_volume_vector, const vector<int>& cluster_rep_vector, vector<int>& sorted_cluster_index_vector)
{
	vector<bool> f;
	f.clear();
...
vector<int> compute_smax(Triangulation& triang, map<int, cell_cluster> &cluster_set, const double& mr)
{
	...
	cluster_ids.clear();
	cluster_vol.clear();
	calc_cluster_volume_and_store_with_cluster_rep(triang, cluster_set, cluster_vol, cluster_ids);
