		...
		for(int id = 0; id < 4; ++id)
		{
			...
			if(triang.is_infinite(cit->neighbor(id)))
			{
				...
				for(int i = 1; i <= 3; ++i)
				{
					...
					new_normal = new_normal / CGAL::sqrt(new_normal.x() * new_normal.x() + new_normal.y() * new_normal.y() + new_normal.z() * new_normal.z());
			...
			fit != triang.finite_facets_end(); ++fit)
	{
		...
		for(int i = 1; i < 4; ++i)
		{
			...
			while(! facet_stack.empty())
			{
				...
				if(! found)
				{
					umbrella_vertices.erase(++find(umbrella_vertices.begin(), umbrella_vertices.end(),  u), umbrella_vertices.end());
				}
			...
			eit != triang.finite_edges_end(); ++eit)
	{
		...
		do
		{
			...
			for(; gcirc != begin; ++gcirc)
			{
				...
				double normals_cos =
					cosine(CGAL::cross_product(u->point() - v->point(), w->point() - v->point()),
						   CGAL::cross_product(w->point() - v->point(), z->point() - v->point()));
...
void walk(const Triangulation& triang)
{
			...
			fit != triang.finite_facets_end(); ++fit)
	{
		...
		for(int i = 1; i <= 3; ++i)
		{
			...
			facet_stack.push_back(Oriented_facet(ch, id, edge_index(id, (id+i)%4, j)));
		}
		ch->tag[id] = true;
...
{
	for(FEI eit = triang.finite_edges_begin(); eit != triang.finite_edges_end(); eit ++)
	{
...
void propagate_in_out_marking(Triangulation& triang, vector < CGAL::Triple<Cell_handle, int, int> >& v_stack)
{
	...
	while(! v_stack.empty())
	{
		...
		// Now move around the outside umbrella of vertex untill it hits a cocone facet.
