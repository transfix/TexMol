...
Geometry* Rover3DWidget::getWireCubes()
{
	return &m_WireCubeGeometry;
}

Geometry* Rover3DWidget::getAxes()
{
	return &m_AxesGeometry;
}
...
void Rover3DWidget::prepareAxes()
{
	if (!m_GeometriesAllocated)
	{
		...
		m_AxesGeometry.AllocatePoints(6);
	}
...
void Rover3DWidget::setBoundary(float* min, float* max)
{
	// get the ratio to update the subcube
	double minx = m_Boundary.getXMin();
	double miny = m_Boundary.getYMin();
	double minz = m_Boundary.getZMin();
	double maxx = m_Boundary.getXMax();
	double maxy = m_Boundary.getYMax();
	double maxz = m_Boundary.getZMax();
	double subminx = m_SubVolume.getXMin();
	double subminy = m_SubVolume.getYMin();
	double subminz = m_SubVolume.getZMin();
	double submaxx = m_SubVolume.getXMax();
	double submaxy = m_SubVolume.getYMax();
	double submaxz = m_SubVolume.getZMax();
	subminx = min[0] + (subminx - minx)*(max[0] - min[0])/(maxx-minx);
	subminy = min[1] + (subminy - miny)*(max[1] - min[1])/(maxy-miny);
	subminz = min[2] + (subminz - minz)*(max[2] - min[2])/(maxz-minz);
	submaxx = min[0] + (submaxx - minx)*(max[0] - min[0])/(maxx-minx);
	submaxy = min[1] + (submaxy - miny)*(max[1] - min[1])/(maxy-miny);
	submaxz = min[2] + (submaxz - minz)*(max[2] - min[2])/(maxz-minz);
	m_Boundary.setExtents(
		min[0], max[0],
		min[1], max[1],
		min[2], max[2]
	);
	updateSubVolume(subminx, subminy, subminz, submaxx, submaxy, submaxz);
	prepareGeometry();
}

void Rover3DWidget::updateSubVolume(double subminx, double subminy, double subminz, double submaxx, double submaxy, double submaxz)
{
	m_SubVolume.setExtents(
		subminx, submaxx,
		subminy, submaxy,
		subminz, submaxz
	);
