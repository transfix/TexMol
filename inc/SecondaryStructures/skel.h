...
class SecondaryStructureData;   /* TexMol visual structure */
class DataManager;
...
class Skel_Polyline
{
	public:
		...
		Skel_Polyline(const int& _nv, const int& _ne)
		{
			nv = _nv;
			ne = _ne;
		}
...
class Skel
{
	public:
		...
		/* inital computations */
		int compute_secondary_structures(Geometry* inputGeom);
		/* additional "interactive computations" */
		HistogramData get_alpha_histogram_data();
		HistogramData get_beta_histogram_data();
		void update_display(Geometry* helixGeom, Geometry* sheetGeom, int alphaCount, int betaCount,
		float alphaMinWidth, float alphaMaxWidth,float betaMinWidth, float betaMaxWidth,bool alphaHistogramChanged);
	...
	private:
		...
		// -- flatness marking ---
		//static const double DEFAULT_ANGLE = M_PI / 8.0; // Half of the co-cone angle.

		// -- robust cocone ---
		//static const double DEFAULT_BIGBALL_RATIO  = (1./4.)*(1./4.); // parameter to choose big balls.
		//static const double DEFAULT_THETA_IF_d  = 5.0; // parameter for infinite-finite deep intersection.
		//static const double DEFAULT_THETA_FF_d  = 10.0; // parameter for finite-finite deep intersection.

		// -- medial axis ---
		//static const double DEFAULT_MED_THETA = M_PI*22.5/180.0; // original: M_PI*22.5/180.0;
		//static const double DEFAULT_MED_RATIO = 8.0*8.0; // original: 8.0*8.0;

		// -- bounding box ---
		vector<double> bounding_box;
		//static const double BB_SCALE = 0;
		...
		/* build geometry */
		void buildSheetGeometry(Geometry* geom);
		void buildHelixGeometry(Geometry* geom);
		void buildSkeletonGeometry(Geometry* geom);
		void buildCurveGeometry(const std::vector< std::vector<int> >& curve_comps, const Curve& curve, Geometry* geom);
};
