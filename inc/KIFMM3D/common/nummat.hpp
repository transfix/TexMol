...
public:
  ...
  NumMat(int m=0, int n=0): _m(m), _n(n), _owndata(true) {
	 if(_m>0 && _n>0) { _data = new F[_m*_n]; assert( _data!=NULL ); std::memset(_data, 0, _m*_n*sizeof(F)); } else _data=NULL;
  }
  ...
  NumMat(int m, int n, bool owndata, F* data): _m(m), _n(n), _owndata(owndata) {
	 if(_owndata) {
		if(_m>0 && _n>0) { _data = new F[_m*_n]; assert( _data!=NULL ); std::memset(_data, 0, _m*_n*sizeof(F)); } else _data=NULL;
		if(_m>0 && _n>0) std::memcpy( _data, data, _m*_n*sizeof(F) );
	 } else {
		_data = data;
	 }
  }
  ...
  NumMat(const NumMat& C): _m(C._m), _n(C._n), _owndata(C._owndata) {
	 if(_owndata) {
		if(_m>0 && _n>0) { _data = new F[_m*_n]; assert( _data!=NULL ); std::memset(_data, 0, _m*_n*sizeof(F)); } else _data=NULL;
		if(_m>0 && _n>0) std::memcpy( _data, C._data, _m*_n*sizeof(F) );
	 } else {
		_data = C._data;
	 }
  }
...
{
  std::memset(M.data(), 0, M.m()*M.n()*sizeof(F));
}
