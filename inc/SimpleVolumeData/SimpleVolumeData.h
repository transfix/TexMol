...
class SimpleVolumeData
{
	public:
		...
		unsigned int parallelGetZOffset() const
		{
			return m_ZOff;
		}
		unsigned int parallelGetDepth() const
		{
			return m_Dims[3];
		}
#endif
		...
		float getSpanX()
		{
			return (m_Max[0] - m_Min[0]) / (float)(m_Dims[0]-1);
		}
		float getSpanY()
		{
			return (m_Max[1] - m_Min[1]) / (float)(m_Dims[1]-1);
		}
		float getSpanZ()
		{
			return (m_Max[2] - m_Min[2]) / (float)(m_Dims[2]-1);
		}
		...
		static bool getUnsignedCharData(void* data1, int type1, void* data2, int type2, void* data3, int type3, void* data4, int type4, unsigned char* data, int width, int height, int depth);
