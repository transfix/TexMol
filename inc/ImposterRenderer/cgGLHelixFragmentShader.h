...
const char cgGLHelixFragmentShader[] =
	...
	"			uniform float3 silhouettecolor,\n"
	...
	"\n"
	"	float opacity = tex2D(opacitymap, IN.texcoord3.xy);\n"
	"	float silhouette = tex2D(silhouettemap, IN.texcoord3.xy);\n"
	"	//float4 outcolor = float4(color, opacity * normal.w);\n"
	"	float4 outcolor = float4(color*opacity*(1.f-silhouette) + silhouette*silhouettecolor, silhouette + opacity);\n"
