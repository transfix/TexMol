...
void ComputeDistanceFunction(float* distanceD,  float* init,  int wid, int nnx, int nny, int nnz, float ddx, float ddy, float ddz, float xS, float yS, float zS)
{
...
void  InitialData_IrBox(float* v, float minx, float maxx, float miny, float maxy, float minz, float maxz, int nx, int ny, int nz)
//              float   *v,    /* the volume data of initial signed distance function */
//              float   minx   /* left end-point of x diection                        */
//              float   maxx   /* right end-point of x diection                       */
//              float   miny   /* left end-point of y diection                        */
...
void    ReInitilazation(float* fun, float* coeff, float dx, float dy, float dz, int nx, int ny, int nz)
//              float   *fun   /* the volume data of initial signed distance function */
//              float   *coeff /* spline coefficients                                 */
//              float   dx     /* x direction spacing                                 */
//              float   dy     /* y direction spacing                                 */
			...
			for(j = 0; j < ny; j++)
			{
				for(k = 0; k < nz; k++)
				{
							...
							fabs(fvalue) < Height)
					{
						...
						{
							sd = fvalue*(3*width*width - fvalue*fvalue)/(2*width*width*width);    // Xu's
						}
						sdp = sd;
						if(sd < 0.0)
						{
